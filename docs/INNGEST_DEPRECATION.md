...
export const generateKeywordContent = inngest.createFunction(
  ...
  { event: 'calendar/keyword.generate' },
  async ({ event, step }) => {
    console.log(`⚠️ DEPRECATED: Use /api/calendar/generate instead`);
...
✅ Uses multi-phase generation (4 phases)
✅ Generates 6,000-8,500 word articles
✅ Includes DataForSEO keyword integration
...
1. **`generateKeywordContent`** - ⚠️ DEPRECATED
   - Now just logs the event
   - Does NOT generate content
   - Prevents duplicate generation

2. **`dailyContentGeneration`** - 🕐 Cron job (6 AM daily)
...
3. **`scheduleKeywordGeneration`** - 📅 Event-based scheduling
...
```
⚠️ DEPRECATED: generateKeywordContent called for: [keyword]
⚠️ This function is deprecated. Use /api/calendar/generate instead
```
